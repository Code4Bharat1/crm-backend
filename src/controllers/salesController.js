...
export const getSalesPerformance = async (req, res) => {
  try {
    ...
    const salesOrders = await SalesOrder.aggregate([
      { $match: { status: { $ne: 'Cancelled' } } },
      {
        $group: {
          ...
          totalOrdersAmount: { $sum: '$grandTotal' },
          orderCount: { $sum: 1 }
        }
      }
    ]);
    ...
    // 3. Aggregate quotations grouped by salesperson: total quotations and customer-confirmed quotations
    ...
    const salespeople = employees.map(emp => {
      ...
      const orderData = ordersMap.get(nameKey) || { totalOrdersAmount: 0, orderCount: 0 };
      const quoteData = quotationsMap.get(nameKey) || { quotationCount: 0, totalQuoted: 0, confirmedCount: 0, confirmedQuotationAmount: 0 };
      ...
      // Achieved revenue is strictly driven by the salesperson's Quotation amounts:
      // 1. If customer confirmed/approved quotations exist (status Accepted/Confirmed or converted to SO/PI),
      //    it counts the customer-approved confirmed quotation value.
      // 2. If no quotations are confirmed yet, it reflects the quotation amount generated by the salesperson.
      // 3. Unlinked standalone sales orders (like ₹4.1L legacy data) are decoupled so they never displace quotation values.
      const confirmedQuoteAmount = quoteData.confirmedQuotationAmount || 0;
      const confirmedQuoteCount = quoteData.confirmedCount || 0;
      const totalQuotedAmount = quoteData.totalQuoted || 0;

      const achieved = confirmedQuoteAmount > 0 
      ...
      return {
        ...
        confirmedQuotations: confirmedQuoteCount,
        confirmedQuotationValue: confirmedQuoteAmount,
    ...
    const totalConfirmedQuotations = salespeople.reduce((acc, s) => acc + (s.confirmedQuotations || 0), 0);
    const totalConfirmedQuotationAmount = salespeople.reduce((acc, s) => acc + (s.confirmedQuotationValue || 0), 0);
    ...
    const teamComparison = salespeople.map(s => ({
      ...
      confirmedQuotations: s.confirmedQuotations,
      confirmedQuotationValue: s.confirmedQuotationValue,
      pct: s.pct
    }));
    ...
    res.json({
      ...
      data: {
        summary: {
          ...
          totalWonLeads,
          totalQuotations,
          totalConfirmedQuotations,
          totalConfirmedQuotationAmount
        },
        salespeople,
        teamComparison
      }
...
/**
 * Returns list of REAL active salespeople only (filtering by sales role or sales department).
 */
export const getSalespeopleList = async (req, res) => {
  try {
    let employees = await Employee.find({
      isActive: true,
      $or: [
    ...
    }).select('_id employeeCode firstName lastName fullName role department email phone').sort({ fullName: 1 }).lean();

    // Safe fallback: if none specifically tagged with sales, return all active employees
    if (!employees || employees.length === 0) {
      employees = await Employee.find({ isActive: true })
        .select('_id employeeCode firstName lastName fullName role department email phone')
        .sort({ fullName: 1 })
        .lean();
    }
    ...
    res.status(500).json({ success: false, message: 'Server error fetching salespeople', error: error.message });
  }
};
